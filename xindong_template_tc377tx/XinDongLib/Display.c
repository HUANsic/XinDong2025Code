...
{
    /*I2C初始化*/
    OLED_I2C.scl_port = I2C_SCL_PORT;
    OLED_I2C.scl_pin = I2C_SCL_PIN;

    OLED_I2C.sda_port = I2C_SDA_PORT;
    OLED_I2C.sda_pin = I2C_SDA_PIN;

    OLED_I2C.status = EI2C_Status.NOT_READY;
    /*I2C初始化*/
    EI2C_Init(&OLED_I2C);
}
...
{
    EI2C_Mem_Write(&OLED_I2C, OLED_I2C_ADDRESS, 0x00, &Command, 1);
}
...
/**
  ...
  * 说    明：此函数用于向OLED发送数据
  */
void OLED_WriteData(uint8 *Data, uint16 Length)
{
    EI2C_Mem_Write(&OLED_I2C, OLED_I2C_ADDRESS, 0x40, Data, Length);
}
...
/**
  ...
  */
void OLED_SetCursor(uint8 Page, uint8 X)
{
...
/**
  ...
  */
uint8 OLED_pnpoly(uint8 nvert, int16 *vertx, int16 *verty, int16 testx, int16 testy)
{
	int16 i, j, c = 0;
...
/**
  ...
  */
uint8 OLED_IsInAngle(int16 X, int16 Y, int16 StartAngle, int16 EndAngle)
{
	int16 PointAngle;
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowSignedNum(uint8 X, uint8 Y, int32 Number, uint8 Length, uint8 FontSize)
{
	uint8 i;
	uint32 Number1;
...
{
	int16 x, y, dx, dy, d, incrE, incrNE, temp;
	int16 x0 = X0, y0 = Y0, x1 = X1, y1 = Y1;
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_DrawArc(uint8 X, uint8 Y, uint8 Radius, int16 StartAngle, int16 EndAngle, uint8 IsFilled)
{
	int16 x, y, d, j;
